...
.onAttach <- function(libname, pkgname){
    ...
    if (!file.exists(db_file) | !file.exists(metadata_file)) {
        packageStartupMessage("SILVA 128.1 database data not present, use `make_silva128.R` in the package inst/scripts directory to download the database into the package inst/extdata/ directory and reinstall the package")
    }
}
...
.onLoad <- function(libname, pkgname){
    ...
    message("Note that - there are 19,22,213 sequences in SILVA v.128 database, of which 29,467 sequences do not have RNACentral and NCBI taxon ID.")
